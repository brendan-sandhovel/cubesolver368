...
# Inside the _gen_solution method of InteractiveCube

    def _gen_solution(self, event):
        # Generate the solution using your existing code
        soln = predict(self._trainCube.stickers)
        
        # Convert to NumPy array and flatten
        soln_array = np.array(soln)
        soln_str = ' '.join(soln_array)
        
        # Display solution in the GUI
        self._soln_label.set_text(f"Solution: {soln_str}")
        self.figure.canvas.draw()
